...
import { contributeScamEvidence } from '../../../lib/shared-scam-evidence';
...
export async function PATCH(request:Request){
 ...
 if(payload.applyToSender){
   ...
   let evidenceContributed=0;if(state==='scam'){for(const record of records.slice(0,25)){try{evidenceContributed+=await contributeScamEvidence(user.email,record);}catch{}}}
   const summary=await getInboxSummary(user.email);await addActivityLog(user.email,'sender_rule_updated',state===null?`Removed the saved sender decision for ${payload.senderEmail}.`:`Marked ${payload.senderEmail} as ${state==='safe'?'a safe sender':'a blocked sender'}.`,{senderEmail:payload.senderEmail,reviewState:state,affectedRecords:records.length,evidenceContributed});
   return NextResponse.json({records,record:records[0],summary,evidenceContributed});
 }
 ...
 let evidenceContributed=0;if(state==='scam'){try{evidenceContributed=await contributeScamEvidence(user.email,record);}catch{}}
 ...
 return NextResponse.json({record,summary,evidenceContributed});
}
