...
export function classifyEmailEvidence(email: EmailInput): ClassificationResult {
  ...
  const authPasses = [spf, dkim, dmarc].filter((state) => state === 'pass').length;
  const authFails = [spf, dkim, dmarc].filter((state) => state === 'fail').length;

  if (authPasses >= 2) addPositive('auth_pass', 18, `${authPasses} email authentication checks passed (SPF/DKIM/DMARC).`, 0.95);
  ...
  if (email.links.length >= 8) addRisk('many_links', 5, 'Message contains many links.', 0.55);
  ...
  const totalEvidence = hits.length + positive.length;
