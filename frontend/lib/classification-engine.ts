...
export function classifyEmailEvidence(email:EmailInput):ClassificationResult{
  // BODY-FIRST RULE: the subject is display/context only and never supplies classification evidence.
  // Risk, message type, and trust decisions are derived from the message body plus sender/auth/link/history evidence.
  const content=String(email.body_text||'').toLowerCase();const hits:RuleHit[]=[];const positive:RuleHit[]=[];let risk=8;
  ...
  const highRiskPayments=containsAny(content,['gift card','bitcoin','crypto wallet','wallet address','wire transfer','western union','moneygram']);if(highRiskPayments.length)addRisk('high_risk_payment',25,'High-risk payment method language detected in the message body.',0.94);
  const credentialTerms=containsAny(content,['password','verification code','security code','one-time code','login now','verify your account','confirm your account']);if(credentialTerms.length)addRisk('credential_request',20,'Credential or account-verification language detected in the message body.',0.9);
  const pressureTerms=containsAny(content,['urgent','immediately','act now','final notice','within 24 hours','suspended','expires today']);if(pressureTerms.length)addRisk('urgency_pressure',Math.min(18,6+pressureTerms.length*4),'Urgency or time-pressure language detected in the message body.',0.82);
  ...
  if(looksLikeSweepstakes)addPositive('sweepstakes_context',10,'Sweepstakes, contest, or entry/rules context was detected in the message body.',0.82);
  if(rewardTerms.length&&!looksLikeSweepstakes)addRisk('reward_claim',Math.min(18,7+rewardTerms.length*3),'Prize, reward, grant, or unexpected-payment language detected in the message body.',0.78);else if(rewardTerms.length&&looksLikeSweepstakes)addRisk('sweepstakes_reward_language',Math.min(8,2+rewardTerms.length*2),'Prize or reward language is present in the body, but it appears in a sweepstakes context.',0.6);
  const authorityTerms=containsAny(content,['irs','department of justice','fbi','social security administration','government grant','federal compensation','compensation fund','supreme court','chief justice','chief judge','world bank','usa bank','release approval order']);if(authorityTerms.length&&FREE_MAIL.has(senderDomain))addRisk('authority_free_mail',30,'Authority or government language in the message body is paired with a free-email sender.',0.98);
